...
import { styleBuildIn } from '../../dist/core/method/style-build-in-helper.js';
...
function isElter(filePath: string): boolean {
  ...
  const checker = (node: ts.Node): boolean => {
    if (ts.isPropertyAccessExpression(node) && ts.isIdentifier(node.name)) {
      ...
      return expressionText === 'elter' && ['create', 'global', 'root', 'style'].includes(methodName);
    }
...
(async () => {
  ...
  const styleFiles = files.filter(isElter);
